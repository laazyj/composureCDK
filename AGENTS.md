...
- Nothing at module scope in `src/` may mutate shared state. Every package that ships a `dist` declares `"sideEffects": false`, which promises a bundler that importing a module and not using it cannot change program behaviour — so a top-level statement that registers, mutates a global, or patches a prototype makes that promise false, and the module can then be dropped from a bundle with its effect. Inert data at module scope is fine (`Duration.minutes(1)`, `new RegExp(...)`, a `Symbol.for(...)` brand, a defaults object). The trap is CDK constructors that register into a static: `new Runtime(...)` appends to `Runtime.ALL`, so it is built inside the function that needs it (see `packages/ses/src/activation.ts`). `examples` and `module-compat` are exempt — neither declares an `exports` map, and neither is resolvable as a dependency. **Nothing enforces this yet** (#481).
