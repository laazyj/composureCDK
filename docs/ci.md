...
- **Fork PRs cannot deploy** — `deploy-test.yml` auto-runs on PRs touching `packages/examples/**`, but a PR from a fork is structurally unable to deploy: it triggers on `pull_request` (never `pull_request_target`), so GitHub issues a read-only token with no `id-token: write` and no secrets, and the OIDC `sub` is pinned to `repo:<org>/<repo>:environment:sandbox` so STS would reject a fork's token regardless. As defence in depth, both jobs carry an `if` guard that skips the workflow entirely when `github.event.pull_request.head.repo.full_name != github.repository`. Do not switch this trigger to `pull_request_target` or broaden the role's `sub` condition.
