...
import type { IInstance, Instance, InstanceProps, InstanceType } from "aws-cdk-lib/aws-ec2";
...
function isBurstableInstanceType(instanceType: InstanceType): boolean {
...
export function resolveInstanceAlarmDefinitions(
  ...
  if (config?.attachedEbsStatusCheckFailed !== false) {
    const cfg = resolveAlarmConfig(
      config?.attachedEbsStatusCheckFailed,
      INSTANCE_ALARM_DEFAULTS.attachedEbsStatusCheckFailed,
    );
    definitions.push({
