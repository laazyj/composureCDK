...
export interface InstanceBuilderResult {
  /** The EC2 Instance construct created by the builder. */
  instance: Instance;
...
class InstanceBuilder implements Lifecycle<InstanceBuilderResult> {
  ...
  private readonly customAlarms: AlarmDefinitionBuilder<Instance>[] = [];
  private _vpc?: Resolvable<IVpc>;
  ...
  build(scope: IConstruct, id: string, context?: Record<string, object>): InstanceBuilderResult {
    const ctx = context ?? {};
    const resolvedVpc = this._vpc ? resolve(this._vpc, ctx) : undefined;
