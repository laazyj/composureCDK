...
| [`ComposureCDK-EventStoreStack`](src/event-store-app.ts)                                            | `createTableV2Builder` event store with a GSI, TTL, and a change stream feeding a Lambda projector; opt-in global-table replicas via `-c ddbReplicaRegions=…` |
| [`ComposureCDK-LedgerTableStack`](src/ledger-table-app.ts)                                          | Classic `createTableBuilder` ledger with provisioned capacity and a customer-managed KMS key, overriding the on-demand/AWS-managed defaults (ADR-0009)        |
