...
| [`ComposureCDK-OrderProcessorStack`](src/order-processor-app.ts)                                    | SNS intake topic fanning out to an SQS work queue that feeds a Lambda consumer, with a caller-owned dead-letter queue on the subscription and recommended alarms wired to a sibling SNS alert topic                           |
...
| [`ComposureCDK-CrudApiStack`](src/crud-api-app.ts)                                                  | Minimal CRUD REST API backed directly by DynamoDB via `AwsIntegration` — no Lambda in the request path, wired with `combine` and consumer-side `tableGrants`, with the table encrypted by a composed customer-managed KMS key |
