...
| [`ComposureCDK-OrderProcessorStack`](src/order-processor-app.ts)                                    | SNS intake topic fanning out to an SQS work queue that feeds a Lambda consumer, which classifies each order note with a Bedrock model through a global inference profile, with model invocation logging on; a caller-owned dead-letter queue on the subscription, and recommended queue, function and model alarms wired to a sibling SNS alert topic |
