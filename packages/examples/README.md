...
| [`ComposureCDK-DynamoStreamProcessorStack`](src/dynamo-stream-processor-app.ts)                     | DynamoDB stream feeding a Lambda consumer via `dynamoEventSource` with bisect-on-error, bounded retries, and an `onFailure` SQS DLQ; recommended alarms wired to a sibling SNS alert topic |
