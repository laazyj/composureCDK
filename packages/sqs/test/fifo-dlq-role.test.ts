...
import { expectSharedSecureDefaults, setUntypedProp } from "./_helpers.js";

const createFifoDlqBuilder = () => createQueueBuilder("fifo-dlq");

const buildAndSynth = buildFixture(createFifoDlqBuilder, "OrderEventsDlq");

describe('createQueueBuilder("fifo-dlq")', () => {
  it("combines the FIFO surface with the DLQ defaults", () => {
    const { template } = buildAndSynth((b) => b.queueName("order-events-dlq.fifo"));
